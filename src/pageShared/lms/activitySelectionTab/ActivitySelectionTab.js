...
const dayArrData = [
    {
        ...
        "name": "Customer",
        "nameColor": "#FFFFFF",
        "routeName": "CustomerListTab"
    },
    {
        "backgroundColor": "#05D093",
        "dataId": 1,
...
class ActivitySelectionTab extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dayData: dayArrData,
    ...
    onLoad = async () => {
        ...
        // } else {
        // await this.getDashboardTabsData();
        // }
    ...
    getDashboardTabsData = async () => {
        let responseData = await MiddlewareCheck("employeeDashboard", {}, this.props);
        if (responseData) {
    ...
    onViewDesign = () => {
        ...
        for (let i = 0; i < this.state.dayData.length; i++) {
            ...
            respDesign.push(
                <TouchableOpacity disabled={isDisable} onPress={() => this.selectItem(itemData, i)}
                    style={{
...
export default ActivitySelectionTab;