...
const loadImagesInBackground = async (catalogue) => {
    // Example: Simulate image loading delay
    return await Promise.all(
        catalogue.map(item => {
            return new Promise((resolve) => {
                setTimeout(() => {
                    resolve({ ...item, imageLoaded: true }); // Simulating image loading completion
                }, 1000);
            });
        })
    );
        ...
        if (responseData) {
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                ...
                InteractionManager.runAfterInteractions(() => {
                    loadImagesInBackground(modifyResponseData.listData).then(loadedCatalogue => {
                        this.setState({
                            listData: [...this.state.listData, ...loadedCatalogue],
                            // imagesLoaded: true,
                        });
                    });
                });
